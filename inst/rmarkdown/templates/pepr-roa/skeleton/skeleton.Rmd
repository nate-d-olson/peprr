...
```{r echo = FALSE, fig.align='center', fig.width = 5, fig.show = 'asis', fig.cap= "Optical mapping measurement process.[^opmap-diagram]"}
...
```{r echo = FALSE,child='rm_specific_stability_experimental_design.Rmd' }
## Add text specific to rm regarding stability experiment
```

__TODO Was this done for the microbial samples as well TODO__
...
To assess the stability of the DNA fragment size distribtion the proportion of DNA in a PFGE gel lane for each sample was determined for two regions between ladder markers. The high region is the proportion of DNA between the 48.5kb and 197kb markers and the low region between 9.24kb and 48.5kb markers. Treatment proportions were compared to controls using a T-test. The gels were processed using the R package peprrDnaStability ([www.github/nate-d-olson/pepr](http://www.github/nate-d-olson/pepr)). Prior to processing the gels were first manually cropped to remove regions of the gel to the left and right of the outer lanes. The `runfindMarkers` starts up an interactive application to define the parameters used in processing the gels (Figure **XYZ**). Croped and original gel images as well as the parameter set used to analyze the gels are available at __%%TODO%%__.

__TODO Screenshot of Shiny App TODO__

#### Stability Study Results
```{r echo = FALSE,child='rm_specific_stability_results.Rmd' }

...
.cbtsv_tosql <- function (tsv_file,db_conn,dplyr::tbl_name) {
  ...
  dplyr::copy_to(vcf_db, vcf, name =dplyr::tbl_name, temporary = FALSE,
...
.pur_tbl <- function (vcf_tbl, db_conn,dplyr::tbl_name) {
  ...
  vcf_dp4 <-dplyr::tbl(src = vcf_db, from = vcf_tbl)  %>%
  ...
  vcf_dp4 <- vcf_dp4  %>%
    ...
    dplyr::mutate(Ref_For = as.numeric(Ref_For),
           ...
           Alt = Alt_For + Alt_Rev,
           Pur = Ref/(Ref+ Alt))
  dplyr::copy_to(db_conn, vcf_dp4, name=tbl_name,temporary = FALSE, indexes = list("CHROM","POS","SAMPLE"))
}

# Generating purity by platform summary
.pur_plat <-function(pur_plat_tbl, db_conn,tbl_name){
 dplyr::tbl(src = vcf_db, from=pur_plat_tbl) %>%
...
.pur_plat_join <- function(pur_plat_tbl1, pur_plat_tbl2, vcf_db,tbl_name,
                           plat1_name = "plat1", plat2_name = "plat2"){
  plat1_tbl <-dplyr::tbl(src = vcf_db, from = pur_plat_tbl1) %>%
    dplyr::group_by(CHROM,POS) %>%
    dplyr::rename(plat1=Pur) %>%
    dplyr::select(CHROM, POS, plat1)
  plat2_tbl <-dplyr::tbl(src = vcf_db, from = pur_plat_tbl2) %>%
    dplyr::group_by(CHROM,POS) %>%

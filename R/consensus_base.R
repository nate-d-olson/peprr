...
parse_vcf_filename <- function(vcf_filename){
    ## TO DO need to update for new pipeline format
  full_split <- stringr::str_split(vcf_filename,pattern = "mpileup_vcf//")[[1]]
  if(grepl("MiSeq",full_split[1])){
    sample_name <- stringr::str_split(full_split[2],"_")[[1]]
...
#
  #generate datatable
  vcf_tbl <- data.table(CHROM = str_sub(string = rownames(info(vcf)),
                                        start = 1,end = 8),
                        POS = ranges(vcf)@start, WIDTH = ranges(vcf)@width,
                        INDEL=info(vcf)$INDEL, DP = info(vcf)$DP,
